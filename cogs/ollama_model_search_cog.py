...
BASE_OLLAMA_URL = "https://ollama.com"
...
class PaginatedEmbed:
    """
    Helper class to create paginated embeds with navigation controls.
    Displays search results across multiple pages with interactive buttons.
    """
    
    def __init__(self, ctx: Context, data: List[Dict], per_page: int = 3):
        """
        Initialize the paginated embed.
        
        Args:
            ctx: The command context
            data: List of model data to paginate
    ...
    def get_page_content(self) -> discord.Embed:
        ...
        Returns:
            discord.Embed: The embed for the current page
        """
    ...
    async def start(self) -> None:
        ...
        Send the initial embed and add reaction controls.
        """
    ...
    async def reaction_handler(self) -> None:
        """
        Handle reactions to navigate through pages or delete the pagination message.
        """
...
class OllamaModelSearchCog(commands.Cog, name="OllamaSearch"):
    ...
    Provides commands to search and display Ollama models with pagination.
    """

    def __init__(self, bot: commands.Bot):
        """
        Initialize the Ollama Model Search cog.
    ...
    async def initialize_session(self) -> None:
        """
        Creates the aiohttp session for web requests.
        """
    ...
    async def _fetch_search_page(self, query: str) -> Optional[str]:
        ...
        Returns:
            str: The HTML content, or None if fetching failed
        """
    ...
    def _parse_search_results(self, html_content: str) -> List[Dict[str, str]]:
        """
        Parses the HTML to extract model information.
        ...
        try:
            ...
            if not result_containers:
                logger.warning("No result containers found. Check selector or page structure.")

            for container in result_containers:
                model_data = {}
                try:
                    # Find the main link within the list item
                    link_tag = container.find('a', class_='group', href=True)
                    if not link_tag:
                    ...
                    if name_span:
                        ...
                        logger.warning(f"Could not find name span within link: {link}")
                        # Fallback using link path
                        parsed_link = urllib.parse.urlparse(link)
                        path_parts = [p for p in parsed_link.path.split('/') if p]
                        if len(path_parts) >= 1:
                            model_data['name'] = path_parts[-1]
                            ...
                            if len(path_parts) > 1 and path_parts[0] != 'library':
                                ...
                                model_data['author'] = path_parts[1]
                            else:
                                model_data['author'] = 'library'
                        else:
                    ...
                    if stats_container:
                        ...
                        if pulls_span: 
                            pulls = pulls_span.text.strip()
                        if tags_span: 
                            tags_count = tags_span.text.strip()
                        if updated_span: 
                            updated = updated_span.text.strip()

                        logger.debug(f"Extracted Stats - Pulls: {pulls}, Tags: {tags_count}, Updated: {updated}")
                    else:
                        logger.warning(f"Could not find stats container for model {model_data.get('name', 'N/A')}")
                    ...
                    logger.warning(f"Error parsing individual model card: {e}", exc_info=False)
                    continue
    ...
    async def osearch(self, ctx: Context, *, query: Optional[str] = None):
        """
        Searches ollama.com for models and displays results with pagination.
...
async def setup(bot: commands.Bot):
    """
    Load the OllamaModelSearchCog into the bot.
